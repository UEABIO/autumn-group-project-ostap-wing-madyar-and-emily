...
# BUILDING THE FIGURE WITH GGPLOT2----

# creating the plot with hospitalizations
plot_Y <- ggplot(sym_count_Y, 
                 aes(fill = n_pos, 
                     y = symptom, 
                     x = hosp_Y))+ # defining the variables to be used as parameters
  geom_col(position ='stack', 
           width = 1.0, 
           color = "black", 
           show.legend = FALSE)+ # selecting a stacked bar-plot with borders, removing the legend as it will be shown on the second plot
  ...
  labs(x = "Number of cases that led to Hospitalizations", # labeling the plot axes
       y = "Symptom",
       title = "Occurrence of COVID-19 Symptoms leading to hospitalizations", #providing a title and subtitle 
       subtitle = "Coughing symptoms were present in the majority of hospitalizations")+
  theme_classic()     # setting a plot theme

# creating the plot without hospitalizations
plot_N <- ggplot(sym_count_N, 
                 aes(fill = n_pos, 
                     y = symptom, 
                     x = hosp_N))+  # defining the variables to be used as parameters
  geom_col(position ='stack', 
           width = 1.0, 
           colour = "black")+  # selecting a stacked bar-plot with borders
  scale_fill_manual(labels = c("With additional Symptoms", "Only specific Symptom"), 
                    values = c("deepskyblue", "darkblue"))+  # labeling the legend, adding color
  scale_y_discrete(labels = N_labs)+  # providing the Y axis labels for the symptoms
  labs(x = "Number of cases that did not lead to Hospitalizations",  # labeling the axes and titling the legend, title omitted as it is already included in the first plot
       y = "",
